...
- Sections:
  • Standings table (Team, W, L, PF, PA, Diff) sorted by Wins desc, then PF desc.
  • Strength of Schedule & Luck table:
      ...
      Luck = W − Exp W.
      Luck badges with ReportLab Paragraph + <font color='...'>:
        🍀 green (> +0.5), 😬 red (< −0.5), ⚖️ gray otherwise.
      Includes “League Avg” SOS row (kept LAST). Sort = Wins desc, then PF desc.
  • Highlights: Heart-Attack Matchups (closest) & Blowouts of the Week (largest).
  • Banter sections (five rotating lines, random.choice):
      Top Dogs (undefeated), Doormats (winless),
      Luckiest (top +Luck), Unluckiest (most −Luck),
      Easiest Schedule (lowest SOS), Hardest Schedule (highest SOS).
  • League Story So Far paragraph.

- Config: environment variable SLEEPER_LEAGUE_ID
         (defaults to 1180243382958317568)
...
def compute_all_play(weekly_points):
    ...
    return {rid: (sum(rates)/len(rates) if rates else 0.0) for rid, rates in per_team_week_rates.items()}
...
# -----------------------------
def main():
    # Use secret if present; otherwise default to your Alliance league
    ...
    try:
        ...
        # Fallback PDF so the workflow still uploads an artifact
    ...
    winless = [r["Team"] for r in standings_rows if r["GP"] > 0 and math.isclose(r["W"], 0.0, rel_tol=1e]()
